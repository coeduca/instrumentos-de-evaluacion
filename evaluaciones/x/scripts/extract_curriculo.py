...
def polish_unit_name(name):
    """Corrige artefactos recurrentes de la capa de texto de los PDF."""
    if not name:
        return name
    # Algunos PDF de Lengua superponen dos veces "Duración", carácter a carácter:
    # "DDuurraacciióónn:: 44 sseemmaannaass".
    name = re.split(
        r'D+u+r+a+c+i+[óo]+n+\s*:*',
        name,
        maxsplit=1,
        flags=re.I,
    )[0].strip(' .:-')
    # Los números romanos en versalitas pueden salir reordenados por posición:
    # "siglos y xix xx" en vez de "siglos xix y xx".
    name = re.sub(
        r'\b(siglos?)\s+y\s+([xvilcdm]+)\s+([xvilcdm]+)\b',
        r'\1 \2 y \3',
        name,
        flags=re.I,
    )
...
def detect_unit(txt, english=False):
    ...
    # Los títulos suelen ocupar dos líneas. La versión anterior capturaba solo
    # la primera y dejaba nombres como "Retos ambientales, alimenticios y de".
    # Se captura hasta el primer rótulo curricular que sigue al nombre.
    stops = (r'Approximate time|UNIT COMPETENC(?:IE|E)S?|Conceptual Content|'
             r'Performance Indicators' if english else
             r'Duración|Eje integrador|Competencias?\s+de\s+(?:la\s+)?unidad|'
             r'Competencia|Tiempo probable|Contenidos')
    m = re.search(
        r'(\d{1,2})\s*\n?\s*'+UWORD+
        r'\s+([A-Za-zÁÉÍÓÚÑ][\s\S]+?)(?=\n\s*(?:'+stops+r')\b)',
        txt,
        re.I if english else 0,
    )
    if not m:
        m = re.search(
            r'\b'+UWORD+r'\s+(?:N[°º]\s*)?(\d{1,2})[:\.\s]+'
            r'([A-Za-zÁÉÍÓÚÑ][\s\S]+?)(?=\n\s*(?:'+stops+r')\b)',
            txt,
            re.I if english else 0,
        )
    if m:
        ...
        name=clean(re.split(stops, m.group(2), maxsplit=1, flags=re.I)[0]).strip(' .:-')
        name=polish_unit_name(name)
        if english:
...
INTRO_MARKERS = [
    ...
    'perfiles de estudiante','perfil de estudiante',
    'ámbitos de la aproximación',
...
def is_unit_page(txt, english=False):
    ...
    if 'indicadores de logro' in low and 'contenidos' in low:
        return True
    # Las páginas de continuación no repiten "Contenidos" ni "Indicadores de
    # logro". Sus filas sí conservan códigos N.N seguidos por un verbo en
    # mayúscula. Algunos programas imprimen además un punto final (N.N.);
    # omitir cualquiera de ambas formas hacía que muchas unidades terminaran
    # artificialmente en 3.4, 4.4, etc.
    return len(re.findall(
        r'(?m)(?:^|\s)\d{1,2}\.\d{1,2}\.?\s+[A-ZÁÉÍÓÚÑ]', txt
    )) >= 1
...
def polish(t, english=False):
    ...
    # Errata tipográfica del programa de Ciencia y Tecnología de octavo:
    # el indicador 7.10 aparece impreso como "dentifica".
    t = re.sub(r'^dentifica\b', 'Identifica', t)
...
def right_col_indicators(page, english=False):
    ...
    EMB = re.compile(r'(?<!\d)(\d{1,2}\.\d{1,2})\s+'+letter)
    split=[]
    for c,t in inds:
        unit_prefix=c.split('.')[0]+'.'
        m=next((m for m in EMB.finditer(t)
                if m.group(1).startswith(unit_prefix)), None)
        while m:
            first=t[:m.start()].strip()
            # Si el código quedó pegado a una sílaba de otra columna
            # ("...comunicativas. don7.8"), conserva solo la oración completa.
            if m.start() and not t[m.start()-1].isspace():
                ends=[first.rfind(mark) for mark in ('. ', '! ', '? ')]
                last=max(ends)
                if last >= 0:
                    first=first[:last+1].strip()
            split.append((c, first))
    ...
    return [(c,t) for c,t in inds
            if int(c.split('.')[1]) > 0 and len(t)>=8 and ok(t)]
