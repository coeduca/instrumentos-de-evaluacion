...
(function () {
  ...
  function nombreAsignatura(asignatura) {
    return asignatura === 'Matemática' ? 'Matemática / Precalculo' : asignatura;
  }
  ...
  const TEMPLATE = `
    <div class="p-6 space-y-5">
      <p class="text-sm text-slate">
        ...
        <strong class="text-navy">indicadores de logro</strong> que realmente se evaluarán.
        Los objetivos de la unidad son <strong class="text-navy">opcionales</strong>: agrégalos solo si te ayudan a contextualizar la actividad.
      </p>
      ...
      <div data-cur="objetivo-wrap" class="hidden bg-navy-50 border border-navy/10 rounded-sm p-4">
        <span class="field-label mb-1">Objetivos / competencias de la unidad <span class="optional-inline">Opcional</span></span>
    ...
    <div class="border-t border-navy/10 bg-paper/60 p-6 space-y-5">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <span class="field-label mb-0">Objetivos de aprendizaje seleccionados <span class="optional-inline">Opcional</span></span>
  ...
  function createCurriculoPicker(rootEl, storageKey, options) {
    ...
    fill(selAsig, Object.keys(CURRICULO).map((a) => ({ value: a, label: nombreAsignatura(a) })), 'Seleccionar');
