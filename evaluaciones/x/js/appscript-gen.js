...
function tablaRubrica(hoja, fila) {
  ...
  for (var k = 0; k < CRITERIOS.length; k++) {
    ...
    var lineasCriterio = contarLineas(cr.texto);
    ...
    if (HAY_DESCRIPTORES) {
      var maxLineasDesc = 1;
      for (i = 0; i < nNiv; i++) {
        maxLineasDesc = Math.max(maxLineasDesc, contarLineas((cr.desc && cr.desc[i]) || ""));
      }
      hoja.setRowHeight(filaDesc, Math.max(36, 18 + Math.max(lineasCriterio, maxLineasDesc) * 11));
      for (i = 0; i < nNiv; i++) {
    ...
    hoja.setRowHeight(filaChk, HAY_DESCRIPTORES ? 20 : Math.max(30, 18 + lineasCriterio * 11));
    for (i = 0; i < nNiv; i++) {
...
function tablaCotejo(hoja, fila) {
  ...
  for (var k = 0; k < CRITERIOS.length; k++) {
    var fondo = (k % 2 === 1) ? C.FONDO : C.BLANCO;
    hoja.setRowHeight(fila, Math.max(30, 18 + contarLineas(CRITERIOS[k].texto) * 11));
