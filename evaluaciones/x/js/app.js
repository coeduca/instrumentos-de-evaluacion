...
function bindConfigFields() {
  ...
  const firmarDocumentos = document.querySelectorAll('.cfg-firmar-documentos');
  firmarDocumentos.forEach((switchFirma) => {
    switchFirma.checked = state.configuracion.firmarDocumentos !== false;
    switchFirma.addEventListener('change', () => {
      state.configuracion.firmarDocumentos = switchFirma.checked;
      firmarDocumentos.forEach((otroSwitch) => { otroSwitch.checked = switchFirma.checked; });
      saveState();
    });
  });
...
async function collectBackupData() {
  ...
  return {
    ...
    localStorage: ls,
    expediente: window.Expediente ? await window.Expediente.exportAll() : [],
    procesos: window.Procesos ? await window.Procesos.exportAll() : [],
  };
}
...
async function createBackupBlob() {
  ...
  return { data, blob: await gzipText(JSON.stringify(pkg)), filename: `Respaldo_COEDUCA_${hoy}.coeduca` };
}
...
async function parseBackupFile(file) {
  ...
  const data = parsed && parsed.formato === BACKUP_FORMAT_COMPRESSED ? restoreCompressedBackupPackage(parsed) : parsed;
