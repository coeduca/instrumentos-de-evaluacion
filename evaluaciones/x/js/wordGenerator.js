// =========================================================
// Generación de Word (.docx) — actividad de recuperación + instrumento; acta de compromiso opcional
...
(function () {
  ...
  const ALERT = 'B54708';
  const LINE = 'C9D2DA';
  const TRANSPARENT_PIXEL = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';
  ...
  async function prepararFirmaDocumento(config) {
    if (window.FirmasDocumento && typeof window.FirmasDocumento.preparar === 'function') {
      await window.FirmasDocumento.preparar(config);
    }
  }

  function firmaPara(nombre, config) {
    if (!window.FirmasDocumento || typeof window.FirmasDocumento.obtener !== 'function') return null;
    return window.FirmasDocumento.obtener(nombre, config);
  }

  function signatureCell(role, name, config) {
    const firma = firmaPara(name, config);
    const imageData = firma
    ...
    const kids = [
      new Paragraph({
        ...
        // La imagen transparente reserva la misma altura cuando no hay firma.
        children: [new ImageRun({
          type: 'png',
          data: imageData,
          transformation: { width: 84, height: 47 },
        })],
      }),
    ];
    if (name) {
  ...
  // Firmas institucionales del num. 15.d + firma del estudiante únicamente
  // porque este documento es un compromiso interno opcional.
  function buildSignatureBlock(config) {
    ...
    return [
      ...
      sigRow([
        signatureCell('Director/a', config.director, config),
        signatureCell('Docente responsable de grado', config.docente, config),
        signatureCell('Representante del Equipo de Evaluación', config.evaluador, config),
      ]),
      sigGroupLabel('RECIBIDO Y COMPROMISO'),
      sigRow([
        signatureCell('Firma del estudiante', '', config),
        emptySigCell(),
        emptySigCell(),
      ]),
    ];
  ...
  // Constancia de notificación: el num. 15.d exige las firmas del director,
  // docente responsable de grado y representante del Equipo de Evaluación.
  // No exige una firma adicional del estudiante o de la familia.
  function buildNotificationSignatureBlock(config) {
    config = config || {};
    return [
      sigGroupLabel('FIRMAS DE LA CONSTANCIA DE NOTIFICACIÓN — Normativa de Evaluación, num. 15.d'),
      sigRow([
        signatureCell('Director/a', config.director, config),
        signatureCell('Docente responsable de grado', config.docente, config),
        signatureCell('Representante del Equipo de Evaluación', config.evaluador, config),
  ...
  function buildOptionalClosureSignatureBlock(config) {
    return [
      sigGroupLabel('ELABORADO POR'),
      sigRow([
        signatureCell(`Docente${config.materia ? ' de ' + config.materia : ''}`, config.docente, config),
        emptySigCell(),
        emptySigCell(),
      ]),
    ];
  ...
  function instrumentTextRuns(text, opts = {}) {
    const lineas = String(text || '').replace(/\r\n?/g, '\n').split('\n');
    return lineas.map((linea, index) => new TextRun({
      text: linea,
  ...
  function instHeaderCell(text) {
    return new TableCell({
      ...
      children: [new Paragraph({ children: instrumentTextRuns((text || '').toUpperCase(), { bold: true, size: 14, color: SLATE }) })],
    });
  }
  ...
  function instCell(text, opts = {}) {
    return new TableCell({
      ...
      children: [new Paragraph({
        ...
        children: instrumentTextRuns(text, opts),
      })],
    });
  }
  ...
  const DOCX_SIDE_IMAGE_MAX_PX = 280;
  const DOCX_CONTENT_WIDTH_PX = 720;

  function buildActivityTextWord(act) {
    ...
    return paragraphs.map((runs) => new Paragraph({
      spacing: { after: 60, line: 235 },
      children: runsToTextRuns(runs),
    }));
  }

  function fitImageDimsToWidth(dims, maxWidth) {
    if (!dims || !dims.w || dims.w <= maxWidth) return dims;
  ...
  function buildActivityImageWord(act, sideBySide) {
    if (!act.imagen || !act.imagen.data) return null;
    const m = /^data:image\/(\w+);base64,(.+)$/.exec(act.imagen.data);
    if (!m) return null;

    let dims = computeImageDims(act.imagen.w, act.imagen.h, act.imagen.size);
    if (sideBySide) dims = fitImageDimsToWidth(dims, DOCX_SIDE_IMAGE_MAX_PX);

    const imageParagraph = new Paragraph({
      alignment: sideBySide ? AlignmentType.CENTER : AlignmentType.LEFT,
      spacing: { before: sideBySide ? 0 : 80, after: act.imagen.caption ? 15 : (sideBySide ? 0 : 140) },
      children: [new ImageRun({
    ...
    const children = [imageParagraph];
    if (act.imagen.caption) {
      children.push(new Paragraph({
        alignment: sideBySide ? AlignmentType.CENTER : AlignmentType.LEFT,
        spacing: { after: sideBySide ? 0 : 140 },
        children: [new TextRun({ text: act.imagen.caption, italics: true, size: 15, color: SLATE })],
      }));
    }

    return { dims, children };
  }

  function buildActivitySideBySideWord(act, textChildren, imagePosition) {
    const imageBlock = buildActivityImageWord(act, true);
    if (!imageBlock) return null;

    const imagePct = Math.max(20, Math.min(42,
      Math.round(((imageBlock.dims.w + 24) / DOCX_CONTENT_WIDTH_PX) * 100)));
    const textPct = 100 - imagePct;

    const textCell = new TableCell({
      width: { size: textPct, type: WidthType.PERCENTAGE },
      verticalAlign: VerticalAlign.TOP,
      borders: { top: NO_BORDER, bottom: NO_BORDER, left: NO_BORDER, right: NO_BORDER },
      margins: {
        top: 0, bottom: 0,
        left: imagePosition === 'right' ? 0 : 120,
        right: imagePosition === 'right' ? 120 : 0,
      },
      children: textChildren,
    });

    const imageCell = new TableCell({
      width: { size: imagePct, type: WidthType.PERCENTAGE },
      verticalAlign: VerticalAlign.TOP,
      borders: { top: NO_BORDER, bottom: NO_BORDER, left: NO_BORDER, right: NO_BORDER },
      margins: {
        top: 0, bottom: 0,
        left: imagePosition === 'right' ? 120 : 0,
        right: imagePosition === 'right' ? 0 : 120,
      },
      children: imageBlock.children,
    });
  ...
  function buildInstructivoContent(est, config, actividades, recup, opts) {
    ...
    content.push(new Paragraph({
      spacing: { after: 180 },
      children: [new TextRun({ text: opts.tituloDocumento || 'INSTRUCTIVO DE ACTIVIDADES DE RECUPERACIÓN', bold: true, size: 24, color: NAVY })],
    }));
    ...
    if (opts.notaNormativa) {
      content.push(new Paragraph({ spacing: { after: 140 }, children: [new TextRun({ text: opts.notaNormativa, italics: true, size: 16, color: SLATE })] }));
    }
    ...
    if (!actividades.length) {
      ...
      actividades.forEach((act, i) => {
        ...
        const textChildren = buildActivityTextWord(act);
        const imagePosition = act.imagen && ['left', 'right'].includes(act.imagen.position)
          ? act.imagen.position
          : 'default';

        if (imagePosition !== 'default') {
          const sideBySide = buildActivitySideBySideWord(act, textChildren, imagePosition);
          if (sideBySide) {
        ...
        if (imagePosition === 'default') {
          const imageBlock = buildActivityImageWord(act, false);
  ...
  function downloadBlob(blob, filename) {
    ...
    a.click();
    if (window.COEducaDrive && window.COEducaDrive.shouldUploadDocuments()) {
      window.COEducaDrive.uploadGeneratedDocument(
        blob,
        filename,
  ...
  async function generarWord(state, estudiantes, logoBase64, recup, opts) {
    if (!window.docx) {
    ...
    await prepararFirmaDocumento(config);
    ...
    estudiantes.forEach((est, index) => {
      ...
      children.push(...buildInstructivoContent(est, config, state.actividades, recup, opts));
    ...
    const filename = opts && opts.filename
      ? opts.filename
      : (incluirActa
        ? `Acta de Compromiso (Opcional), Actividad e Instrumento - ${sanitizeFilename(config.trimestre)} - ${sanitizeFilename(config.anio)}.docx`
        : `Actividad e Instrumento de Recuperación - ${sanitizeFilename(config.trimestre)} - ${sanitizeFilename(config.anio)}.docx`);
  ...
  function generarUltimoRecursoWord(state, estudiantes, logoBase64, recup, opts) {
    const config = state.configuracion || {};
    const opciones = {
      ...(opts || {}),
      incluirActa: false,
      tituloDocumento: 'INSTRUCTIVO DE ACTIVIDAD DE ÚLTIMO RECURSO',
      filename: `Actividad de Último Recurso - ${sanitizeFilename(config.materia)} - ${sanitizeFilename(config.grado)} - ${sanitizeFilename(config.anio)}.docx`,
    };
    return generarWord(state, estudiantes, logoBase64, recup, opciones);
  }
  ...
  function buildActaIncumplimientoWord(est, config, actividades, fechaEmision, logoBase64) {
    ...
    content.push(
      ...
      ...buildOptionalClosureSignatureBlock(config),
    );

    return content;
  }

  async function generarActaIncumplimientoWord(state, students, logoBase64) {
    if (!window.docx) {
      alert('No se pudo cargar la librería de Word (docx). Verifica tu conexión a internet e intenta de nuevo.');
      return;
    }
    ...
    const filename = `Actas de Incumplimiento - ${sanitizeFilename(state.configuracion.trimestre)} - ${sanitizeFilename(config.anio)}.docx`;
  ...
  function buildActaResultadoWord(est, config, actividades, recup, fechaEmision, logoBase64) {
    ...
    content.push(...buildOptionalClosureSignatureBlock(config));
    return content;
  }

  async function generarActaResultadoWord(state, students, logoBase64, recup) {
    if (!window.docx) {
      alert('No se pudo cargar la librería de Word (docx). Verifica tu conexión a internet e intenta de nuevo.');
      return;
    }
    ...
    const filename = `Actas de Resultado - ${sanitizeFilename(state.configuracion.trimestre)} - ${sanitizeFilename(config.anio)}.docx`;
  ...
  // =========================================================
  async function generarActasCierreWord(state, students, logoBase64, recup) {
    if (!window.docx) {
      alert('No se pudo cargar la librería de Word (docx). Verifica tu conexión a internet e intenta de nuevo.');
      return;
    }
    ...
    const filename = `Actas de Cierre - ${sanitizeFilename(config.trimestre)} - ${sanitizeFilename(config.anio)}.docx`;
    Packer.toBlob(doc).then((blob) => downloadBlob(blob, filename)).catch((e) => {
  ...
  function buildConstanciaRefuerzoWord(est, config, refuerzo, recup, fechaEmision, logoBase64) {
    ...
    const content = [
      ...
      new Paragraph({ spacing: { after: 160 }, children: [] }),
      centeredRun('CONSTANCIA DE REFUERZO EDUCATIVO Y NOTIFICACIÓN', { bold: true, size: 22, after: 200 }),
      bodyParagraph(parrafo1),
    ];
    ...
    content.push(bodyParagraph(parrafoCierre));
    content.push(...buildNotificationSignatureBlock(config));
    return content;
  }

  async function generarConstanciaRefuerzoWord(state, students, logoBase64, recup) {
    if (!window.docx) {
      alert('No se pudo cargar la librería de Word (docx). Verifica tu conexión a internet e intenta de nuevo.');
      return;
    }
    ...
    const filename = `Constancias de Refuerzo - ${sanitizeFilename(state.configuracion.trimestre)} - ${sanitizeFilename(config.anio)}.docx`;
    Packer.toBlob(doc).then((blob) => downloadBlob(blob, filename)).catch((e) => {
  ...
  // =========================================================
  // CONSTANCIA E INSTRUCTIVO DE ACTIVIDAD DE EVALUACIÓN ORDINARIA (.docx)
  // =========================================================
  function buildActividadOrdinariaWord(config, ordinaria, recup) {
    ...
    content.push(sigRow([signatureCell('Docente', config.docente, config), emptySigCell(), emptySigCell()]));
  ...
  async function generarActividadOrdinariaWord(state, ordinaria, logoBase64, recup) {
    if (!window.docx) {
      alert('No se pudo cargar la librería de Word (docx). Verifica tu conexión a internet e intenta de nuevo.');
      return;
    }
    ...
    const doc = new Document({
      ...
      const filename = `${sanitizeFilename(ordinaria.tipoLabel)} - ${sanitizeFilename(ordinaria.titulo)} (${ordinaria.ponderacion}%).docx`;
  ...
  window.ActasWord = {
    ...
    generarUltimoRecurso: generarUltimoRecursoWord,
  };
})();
