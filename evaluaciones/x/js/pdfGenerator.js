...
function outputPdf(docDefinition, filename, opts) {
  ...
  if (opts && opts.preview) pdf.open();
    ...
    if (window.COEducaDrive && window.COEducaDrive.shouldUploadDocuments()) {
      pdf.getBlob((blob) => window.COEducaDrive.uploadGeneratedDocument(blob, filename, 'application/pdf'));
    }
  }
}
...
function signatureLine(role, name, config) {
  const firma = firmaPara(name, config);
  return {
    width: '*',
    stack: [
      firma
        // La firma es deliberadamente más pequeña y baja 5 pt sobre la línea.
        // El espacio equivalente de la variante vacía mantiene las tres líneas alineadas.
        ? { image: firma.dataUrl, fit: [86, 48], alignment: 'center', margin: [0, 10, 0, -5] }
        : { text: ' ', fontSize: 1, margin: [0, 10, 0, 42] },
...
function buildSignatureBlock(config) {
  ...
  return {
    stack: [
      ...
      {
        columns: [
          signatureLine('Director/a', config.director, config),
          signatureLine('Docente responsable de grado', config.docente, config),
          signatureLine('Representante del Equipo de Evaluación', config.evaluador, config),
        ],
        columnGap: 16,
      },
      ...
      {
        columns: [
          signatureLine('Firma del estudiante', '', config),
          { text: '', width: '*' },
          { text: '', width: '*' },
        ],
        columnGap: 16,
...
async function generarPdf(state, estudiantes, logoBase64, recup, opts) {
...
function generarUltimoRecursoPdf(state, estudiantes, logoBase64, recup, opts) {
  ...
  return generarPdf(state, estudiantes, logoBase64, recup, opciones);
}
...
// =========================================================
function formalSignatureColumn(role, name, config) {
  const firma = firmaPara(name, config);
  return {
    width: '*',
    stack: [
      firma
        ? { image: firma.dataUrl, fit: [92, 52], alignment: 'center', margin: [0, 8, 0, -6] }
        : { text: ' ', fontSize: 1, margin: [0, 8, 0, 45] },
...
function buildOptionalClosureSignature(config) {
  return [
    ...
    { columns: [formalSignatureColumn(`Docente${config.materia ? ' de ' + config.materia : ''}`, config.docente, config), { text: '', width: '*' }, { text: '', width: '*' }], columnGap: 16 },
  ];
}
...
function buildNotificationSignature(config) {
  return [
    ...
    { columns: [formalSignatureColumn('Director/a', config.director, config), formalSignatureColumn('Docente responsable de grado', config.docente, config), formalSignatureColumn('Representante del Equipo de Evaluación', config.evaluador, config)], columnGap: 16 },
  ];
}
...
async function generarActaIncumplimientoPdf(state, students, logoBase64, opts) {
  if (!students.length) return;
  await prepararFirmaDocumento(state.configuracion);
...
async function generarActaResultadoPdf(state, students, logoBase64, recup, opts) {
  if (!students.length) return;
  await prepararFirmaDocumento(state.configuracion);
  const fechaEmision = fechaHoyLarga();
...
// =========================================================
async function generarActasCierrePdf(state, students, logoBase64, recup, opts) {
  if (!students.length) return;
  await prepararFirmaDocumento(state.configuracion);
  const fechaEmision = fechaHoyLarga();
...
async function generarConstanciaRefuerzoPdf(state, students, logoBase64, recup, opts) {
  if (!students.length) return;
  await prepararFirmaDocumento(state.configuracion);
...
async function generarExpedientePeriodoPdf(records, logoBase64) {
  ...
  for (const rec of ordenados) {
    await prepararFirmaDocumento((rec.payload && rec.payload.config) || {});
  }
...
function buildActividadOrdinariaContent(config, ordinaria, recup) {
  ...
  content.push({
    columns: [
      signatureLine('Docente', config.docente, config),
      { text: '', width: '*' },
      { text: '', width: '*' },
    ],
    margin: [0, 16, 0, 0],
...
async function generarActividadOrdinariaPdf(state, ordinaria, logoBase64, recup, opts) {
